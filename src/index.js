import pageLoadfunction from "./pageLoad";

const content = document.querySelector('#content')

const navigation = document.createElement('nav')
navigation.innerHTML = ` <ul class="navigation">
<div class="nav__link"><a href="">HOMEPAGE</a></div>
<div class="nav__link"><a href="">MENU</a></div>
<div class="nav__link"><a href="">HISTORY</a></div>
</ul>`;

const mainContent = document.createElement('main')
mainContent.innerHTML = `<h1>MEAT LINE</h1>
<p class="home__description">Some text about how awesome this restaurant is</p>
<button type="button">Go to menu</button>`

const footer = document.createElement('footer')
footer.innerHTML = `<footer>Made by VoronovichDev</footer>`

window.onload = () => {
   pageLoadfunction(content, navigation, mainContent, footer)
}